Sounda transient_detection ;

void transient_setup() {
  transient_detection = new Sounda(512);
  surface.setSize(transient_detection.buffer_size(), transient_detection.buffer_size());
...
void keep_raw_value() {
  if(raw_value == null) {
    raw_value = new float[transient_detection.buffer_size()];
  }

  for(int i = 0 ; i < transient_detection.buffer_size() ; i++) {
    raw_value[i] = transient_detection.get_left(i);
  }
}
...
void low_pass(float smooth) {
  if(low_pass_value == null) {
    low_pass_value = new float[transient_detection.buffer_size()];
  }

  float ref = transient_detection.get_left(0);
  smoothing = abs(smooth)+1;
  // println("smooth", smoothing);
  for(int i = 0 ; i < transient_detection.buffer_size() ; i++) {
    float current_value = transient_detection.get_left(i);
